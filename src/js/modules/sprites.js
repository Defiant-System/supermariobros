	...
	while(loc < clength) {
		switch(colors[loc]) {
			...
			case 'x':
				...
				// Get the color
				current = Global.makeDigit(paletteref[colors.slice(loc, loc += digitsize)], Global.digitsize);
				// Get the rep times
...
function spriteExpand(colors) {
	...
	// For each number,
	while(i < clength) {
		current = colors.slice(i, i += Global.digitsize);
		// Put it into output as many times as needed
		for(j = 0; j < Global.scale; ++j)
			output += current;
	}
	return output;
}
...
function spriteGetArray(colors) {
	var clength = colors.length,
		numcolors = clength / Global.digitsize,
		split = colors.match(new RegExp('.{1,' + Global.digitsize + '}', 'g')),
		olength = numcolors * 4,
...
function getPaletteReferenceStarting(palette) {
	...
	for(var i = 0; i < palette.length; ++i)
		output[Global.makeDigit(i, Global.digitsize)] = Global.makeDigit(i, Global.digitsize);
	return output;
}
...
function getPaletteReference(palette) {
	...
	for(var i = 0; i < palette.length; ++i)
		output[Global.makeDigit(i, digitsize)] = Global.makeDigit(palette[i], digitsize);
	return output;
}
